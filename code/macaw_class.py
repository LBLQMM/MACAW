...
Contains the Macaw class, the Macaw_optimus function, and the smiles_clean function.
...
class Macaw:
    ...
    def __init__(self, smiles, Nlndmk = 50, fptype = "Morgan2", metric = "Tanimoto", 
                 lndmk_idx = [], Y = [], Yset = 10, edim = 15, method = "MDS"):
        """
        ...
        _fptype : str, optional
            ...
            Options include "RDK5", "RDK7", "Morgan2", "Morgan3", "featMorgan2", 
            "featMorgan3,"Avalon","MACCS", "atompairs", "torsion", "pattern",
            "secfp", and "layered". Defaults to "Morgan2".
            
        _metric : str, optional
            Distrance metric used to measure similarity between molecular fingerprints.
            Options include "Tanimoto", "dice", "cosine", "Sokal", "Kulczynski", 
            "Mcconnaughey", "Braun-Blanquet", "Rogot-Goldberg", "asymmetric", and "Manhattan". 
        ...
        _Y : numpy.ndarray, optional
            Array containing the property of interest for each molecule in the smiles
            input.
        ...
        _Yset : str or int, optional
            ...
            it will use uniform sampling of landmarks after splitting the molecules 
            in `Yset` bins. Defaults to 10.
        ...
        transform(smiles : list, optional)
            Returns the Macaw embedding for the molecules provided in SMILES format.
            If `smiles`is not provided, it will embed the molecules that were provided
            when the Macaw object was created.
        
        set_
            Setter methods are available for the different attributes. See Notes.
        
        
        Notes
        -------
        
        No attribute should be modified directly. Instead, setter methods are implemented 
        for the different attributes, namely `set_Nlndmk()`, 
        `set_lnmk_idx()`, `set_fptype()`, `set_metric()`, `set_Y()`, `set_Yset()`, 
        `set_edim()`, and `set_method()`.
        ...
        # These attributes need not be visible or modified by the user, we make them private
        self.__refD = []
        self.__LndS = []
        self.__refps = []
        ...
        if len(lndmk_idx) == 0:
            ...
            # This will already do the refps_update, refD_update, and safe_lndmk_embed
    ...
    def set_lndmk_idx(self, lndmk_idx):
        ...
        # We are not storing bad smiles/molecules, so we shift the indices accordingly
        # to use the landmarks that the user means
        for i in bad_idx:
    ...
    def __lndmk_choice(self):
        ...
        if lenY != len(mols):  
            ...
            if Yset == "highest": # top sampling
                lndmk_idx = np.argpartition(Y, -Nlndmk)[-Nlndmk:]
            elif Yset == "lowest": # bottom
                lndmk_idx = np.argpartition(Y, Nlndmk)[:Nlndmk]
                ...
                try:
                    ...
                    nbins = min(10, Nlndmk)
                Y_binned = np.floor((Y-min(Y))/(1.00001*(max(Y)-min(Y)))*nbins)
                
                proba = []
                for i in range(lenY):
                    proba.append(1./sum(Y_binned==Y_binned[i]))
                proba = proba/sum(proba)
                lndmk_idx = np.random.choice(range(len(mols)), Nlndmk, 
    ...
    def __lndmk_embed(self):
        ...
        if method == "mds":
            ...
            LndS = Isomap(n_neighbors=n_neighbors, n_components=edim, metric="precomputed")
            LndS.fit(D)
    ...
    def transform(self, qsmiles=[]):
        ...
        if method == "mds":
            ...
            if Lh.shape[1] != D.shape[1]:
                raise ValueError('Must provide distance of each point to every landmark')
                return []
            D = D.T
            N = D.shape[1]
    ...
    def _smiles_to_mols(self, smiles, bad_idx=False):
        ...
        for i in range(len(smiles)):
            m = MolFromSmiles(smiles[i], sanitize=True) # let us implement a more robust strategy
            if m is not None:
                mols.append(m)              
            else:
        ...
        if l1 == 0: # If fps1 is not provided, we return the symmetric matrix
            ...
            for i in range(l2-1):
                for j in range(i+1,l2):
                    s = DataStructs.FingerprintSimilarity(fps2[i],fps2[j], metric=metric)
                    S[i,j] = s
                    S[j,i] = s
        else:           
            S = np.zeros((l1, l2))
    ...
    def __metric_to_class(self):
        ...
        if metric == "tanimoto": r = DataStructs.TanimotoSimilarity
        elif metric == "cosine": r = DataStructs.CosineSimilarity
        elif metric == "dice": r = DataStructs.DiceSimilarity
        # elif metric =="russel": r = DataStructs.RusselSimilarity # non-commutative
        elif metric == "sokal": r = DataStructs.SokalSimilarity
        elif metric == "kulczynski": r = DataStructs.KulczynskiSimilarity
        elif metric == "mcconnaughey": r = DataStructs.McConnaugheySimilarity
        # elif metric == "tversky": r = lambda x, y: DataStructs.TverskySimilarity(x, y, 0.01, 0.99) # non-commutative
        elif metric == "braun-blanquet": r = DataStructs.BraunBlanquetSimilarity
        elif metric == "rogot-goldberg": r = DataStructs.RogotGoldbergSimilarity
        # elif metric == "onbit": r = DataStructs.OnBitSimilarity # Seems the same as Tanimoto
        elif metric == "asymmetric": r = DataStructs.AsymmetricSimilarity
        elif metric == "manhattan": r = DataStructs.AllBitSimilarity
        else: 
            print(f"Warning: Invalid similarity metric {metric}. metric has been set to Tanimoto.")
...
def Macaw_optimus(smiles, y, fast=True, method="regression", C=20., verbose=False, **kwargs):
    """
    Function that identifies and returns the optimal Macaw embedding for a given task.
    ...
    fast : bool, optional
        ...
        and distance metrics, works with a random sample of the dataset provided
        if it is large (>400 points), and uses 3 cv folds instead of 5.
    
    method : str, optional
        Indicates whether it is a "regression" or "classification" problem. It 
        determines if the method to use is a SVR or SVC.
    ...
    **kwargs : optional
    ...
    if fast:
        fptypes = ["morgan2","morgan3","featmorgan3","maccs","atompairs","secfp","torsion","rdk7"]
        ...
        fptypes = ["morgan2","morgan3","rdk5","rdk7","featmorgan2","featmorgan3","maccs","avalon","atompairs","torsion","pattern","secfp","daylight"]
        metrics = ["tanimoto","cosine","dice","sokal","kulczynski","mcconnaughey","braun-blanquet","rogot-goldberg","asymmetric","manhattan"]
    ...
    for i in range(len(fptypes)):
        ...
        for j in range(len(metrics)): 
            ...
            # splitters are instantiated with shuffle=False so the splits will be the same across calls.
            M[i,j] = cross_val_score(f, x, y[idx], cv = cv).mean()
    ...
    # Torsion + sokal can be an invalid combination giving nan. We set its score to 0.0
    np.nan_to_num(M, copy=False)
    max_i,max_j = np.unravel_index(M.argmax(), M.shape)
    
    
    print(f'Setting recommended combination: {fptypes[max_i]} + {metrics[max_j]}')        
    
    # Now we embed all input using the optimal combination
    if ("Y" in kwargs) & (leny > 400) & fast:
        kwargs["Y"] = Y_copy
